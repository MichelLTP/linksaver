...
describe('app routes', () => {
  ...
  it('renders an explicit open-link action for saved products', async () => {
    ...
    const openLinkActions = await screen.findAllByRole('link', { name: /open link/i })
  ...
  it('renders an explicit open-link action on the compare page', async () => {
    ...
    const openLinkAction = await screen.findByRole('link', { name: /open link/i })
  ...
  it('shows a category-focused heading for the category route', async () => {
    ...
    expect(await screen.findByRole('heading', { name: `Category: ${tech.name}` })).toBeInTheDocument()
  })
})
